...
const accountStore = createStore(`${publicSettings.name}_UserAccount`, 'store');
...
const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
	const { user, userLoading, isLoggedIn } = useTracker(() => {
		...
		const getStoredUser = () =>
			get('userId', accountStore)
				.then((result) => cachedUser.set(result ? parse(result) : null))
				.catch(() => cachedUser.set(null));
		...
		const user: IUserProfile | undefined =
			subHandle?.ready() && meteorUser ? userprofileApi.findOne({ email: (meteorUser?.profile as any).email }) : null;

		return {
			user,
			userLoading: hasValue(meteorUser) && !hasValue(user),
			isLoggedIn: !!userId && !!meteorUser
		};
	}, []);
	...
	const handleSignIn = useCallback((email: string, password: string, callBack: (error: IMeteorError) => void) => {
